...
class HouseListHandler(BaseHandler):
    ...
    def get(self):
        ...
        sql = "select distinct hi_house_id,hi_title,hi_price,hi_room_count,hi_order_count,hi_index_image_url,hi_address,up_avatar,hi_order_count,hi_price,hi_ctime from ih_house_info left join ih_order_info on hi_house_id=oi_house_id inner join ih_user_profile on hi_user_id=up_user_id"
        sql += sql_where
        sql += " order by "
        if "booking" == sort_key:
...
page      (p)    int           否        默认第一页
        ...
        # 查询房屋信息
        sql = "select distinct hi_house_id,hi_title,hi_price,hi_room_count,hi_order_count,hi_index_image_url,hi_address,up_avatar,hi_ctime,hi_order_count,hi_price from ih_house_info left join ih_order_info on hi_house_id=oi_house_id inner join ih_user_profile on hi_user_id=up_user_id"
        if sql_where:
            sql += " where "
            sql += " and ".join(sql_where)
